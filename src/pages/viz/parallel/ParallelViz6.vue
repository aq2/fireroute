...
data() {
  return {
    svgHeightRatio: 0.95, headerHeight: 120,
    buttonWidth: 290, sideMarginWidth: 100, topmarginHeight: 60, 
...
methods: {
  ...
  initChart() {
    // set size of svg 
    this.svgHeight = this.windowH - this.headerHeight
    this.svgWidth = this.windowW - this.buttonWidth
    this.chartHeight = this.svgHeight - this.topmarginHeight
    this.chartWidth = this.svgWidth - this.sideMarginWidth
    ...
    this.chartGrp = this.svg
                        .append('g')
                          .attr('class', 'chart')
                          .attr('transform', this.myXY(this.sideMarginWidth/2, this.topmarginHeight/3))
    
    // this.buttonGrp = this.svg
    //                       .append('g')
    //                       .attr('class', 'buttons')
    //                       // .attr
    ...
    // this.buttonGrp.selectAll('rect')
    //               .data(this.superData.dimNames)
    //               .enter()
    //                 .append('rect')
    //                 .attr('class', 'btn')
    //                 .attr('id', (d,i) => 'btn'+i)
    //                 .attr('x', (d, i) => this.xScale(d))
    //                 .attr('y', 300)
    //                 .attr('fill', 'red')
  ...
  setupChart() {
    ...
    //                 .attr('class', 'btn')
    //                 .attr('id', (d,i) => 'btn'+i)
    //                 .attr('x', (d, i) => this.xScale(d) + this.margin/2 -40)
    ...
    this.axesGrp.append('g')
            ...
            .attr('font-size', '24')    // size of dimNames
            .attr('stroke-width', '0')  // remove the xAxis line

    this.yAxesGrp = this.axesGrp.append('g')
                    .attr('class', 'yAxes')

    // todo - enter()?   // use superData instead??
    Object.values(this.superData.dimsAll)
      .forEach((dimObj) => {
        this.yAxesGrp
            .append('g')
